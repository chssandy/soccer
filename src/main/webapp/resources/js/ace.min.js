...
jQuery(function(a){
    ...
    ace.handle_side_menu=function(a){
        ...
        a(".nav-list").on(ace.click_event,function(h){
            ...
            c=a("#sidebar").hasClass("menu-min");  // 判定菜单是否为收缩状态
            if(!g.hasClass("dropdown-toggle")){
                if(c&&ace.click_event=="tap"&&g.get(0).parentNode.parentNode==this){
                    var i=g.find(".menu-text").get(0);
                    if(h.target!=i&&!a.contains(i,h.target)){return false}
                }
                if(b){
                    document.location=g.attr("href");return false
                }
                if(g.parent().parent().hasClass("nav-list")){
                    g.parent().siblings().removeClass("active open");
                    g.parent().siblings().children('ul').children('li').removeClass("active");
                    g.parent().siblings().children('ul').removeAttr("style");
                }
                if(g.parent().parent().parent().parent().hasClass("nav-list")){
                    if(c){
                        g.parent().parent().parent().siblings().removeClass("active open");
                        g.parent().parent().parent().siblings().children('ul').children('li').removeClass("active");
                        g.parent().parent().parent().siblings().children('ul').removeAttr("style");
                        g.parent().parent().parent().addClass("active open");
                    }else{
                        g.parent().parent().parent().siblings().removeClass("active open");
                        g.parent().parent().parent().children('ul').children('li').removeClass("active");
                        g.parent().parent().parent().siblings().children('ul').removeAttr("style");
                        g.parent().parent().parent().addClass("active open");
                    }
            ...
            if(!a(f).is(":visible")){
                ...
                d.find("> .open > .submenu").each(function(){
                    ...
                    if(this!=f&&a(this.parentNode.parentNode).hasClass("active")){
                        a(this).slideUp(200).parent().partent().parent().parent().removeClass("active open")
                    }
                })
            }else{
            ...
            if(c&&a(f.parentNode.parentNode).hasClass("nav-list")){
                ...
                g.parent().siblings().children('ul').children('li').removeClass("active");
                g.parent().siblings().children('ul').removeAttr("style");
                g.parent().children("ul").addClass("showstyle");
                a(f).slideToggle(200).parent().toggleClass("active open");
                return false
